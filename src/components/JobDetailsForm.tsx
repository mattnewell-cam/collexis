...
export default function JobDetailsForm({ job }: { job: Job }) {
  ...
  return (
    <div className="max-w-xl px-8 py-8 space-y-0">
      {/* Contact */}
      <section className="pb-8 border-b border-gray-100 space-y-4">
      ...
      {/* Job */}
      <section className="py-8 border-b border-gray-100 space-y-4">
        ...
        <Field label="Job detail">
          <textarea className={inputCls + ' resize-none'} rows={4} value={draft.jobDetail} onChange={e => set('jobDetail', e.target.value)} />
        </Field>
        <Field label="Invoice total">
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-gray-400">£</span>
            <input type="number" className={inputCls + ' pl-6'} value={draft.price} onChange={e => set('price', parseFloat(e.target.value) || 0)} />
          </div>
        </Field>
      </section>
      ...
      <section className="py-8 border-b border-gray-100 space-y-4">
        ...
        <Field label="Days overdue">
          <input type="number" className={inputCls} value={draft.daysOverdue} onChange={e => set('daysOverdue', parseInt(e.target.value) || 0)} />
        </Field>
        <Field label="Amount paid (£)">
          <input type="number" className={inputCls} value={draft.amountPaid} onChange={e => set('amountPaid', parseFloat(e.target.value) || 0)} />
        </Field>
      </section>
      ...
      <section className="py-8 border-b border-gray-100 space-y-4">
        ...
        <textarea className={inputCls + ' resize-none'} rows={4} value={draft.contextInstructions} onChange={e => set('contextInstructions', e.target.value)} />
      </section>
      ...
      {/* Save bar */}
      <div className="sticky bottom-0 bg-white border-t border-gray-100 -mx-8 px-8 py-4 flex justify-end gap-2">
        <button
