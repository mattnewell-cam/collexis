...
- Optional bug triage now watches persistent `app_logs`, groups distinct failures into `bug_incidents`, asks OpenAI whether each looks like a real bug or a transient issue, and can hand high-confidence incidents to an external runner that opens a draft PR.
